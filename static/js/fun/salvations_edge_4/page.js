// Salvation's Edge helper page script. It drives the click-through decision flow for the encounter helper UI.
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let ico = "";
    let jackpot_num = 0;
    // 각 main_text 요소 클릭은 현재 선택 상태를 바꾸고 안내 텍스트를 다시 그리는
    // 간단한 분기형 상태 머신 역할을 한다.
    mainTextElements.forEach(element => {
        element.addEventListener('click', (event) => {
            // This page is a branching helper: each click advances a small decision tree
            // and rewrites the visible guidance instead of navigating a multi-step form.
            // 클릭된 요소의 id를 가져옵니다.
